...
class MarkovChainLobModel:
    ...
    def all_rates(self):
        ...
        # ) / 2
        lo_rates = ((
            np.array([0.039825, 0.020825, 0.012525, 0.006125, 0.00345])
            + np.array([0.0033, 0.00475, 0.0037, 0.025625, 0.058675])
        ) / 2)*10
        print(f"lo_rates: {lo_rates}")
        ...
        # ) / 2
        canc_rates = ((
            np.array([0.024525, 0.011475, 0.02025, 0.005475, 0.001275])
            + np.array([0.00085, 0.000525, 0.0004, 0.004125, 0.02565])
        ) / 2)*10
        ...
        rates["lo size"] = 0.0005 * 1000
        rates["mo size"] = ((0.007 + 0.003)/2) * 100
